...
(function(f) {
	...
	function A(a) {
		var b = f(a.table().container()),
			...
			e;
		b
			.on('mousedown.dtSelect', d, function(a) {
	...
	function m(a, b, c, d) {
		if (!d || a.flatten().length)
			'string' === typeof b && (b += '.dt'), c.unshift(a), f(a.table().node()).trigger(b, c);
	}
	function B(a) {
		var b = a.settings()[0];
		...
		a.aoRowCreatedCallback.push({
			fn: function(b, d, e) {
				...
				for (e = a.aoColumns.length; b < e; b++)
					(a.aoColumns[b]._select_selected || (d._selected_cells && d._selected_cells[b])) &&
	...
	g.ext.selector.cell.push(function(a, b, c) {
		...
		for (var e = 0, f = c.length; e < f; e++) {
			...
			((!0 === b && g._selected_cells && !0 === g._selected_cells[c[e].column]) ||
